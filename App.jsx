...
const tone=(freq,vol=0.1,dur=0.08)=>{
  try{
...
// ─── Confetti (outside App so stable reference) ───────────────────────────────
...
// ─── Ring (outside App so stable reference) ───────────────────────────────────
function Ring({pct=0,color="#c8ff57",size=96,stroke=9,label,val,onClick}){
  ...
  return(
    <div onClick={onClick} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:7,cursor:onClick?"pointer":"default"}}>
      ...
      <span style={{fontSize:".65rem",color:"#555",textTransform:"uppercase",letterSpacing:".1em",textAlign:"center"}}>{label}</span>
    </div>
  );
}
...
const mkTodayKey = () => DAY_KEYS[(new Date().getDay()+6)%7];
const INIT_TASKS=()=>[
  {id:1,text:"Vacuum living room",folderId:1,recurring:false,day:mkTodayKey(),done:false,timerSeconds:0,timerRunning:false,timerStartedAt:null},
...
.login{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;background:radial-gradient(ellipse at 50% 0%,#c8ff5708 0%,var(--bg) 60%)}
.login-card{background:var(--s);border:1px solid var(--b2);border-radius:24px;padding:52px 44px;max-width:420px;width:100%;text-align:center;box-shadow:0 40px 80px #000a}
.login-logo{font-family:'Syne',sans-serif;font-weight:800;font-size:2.2rem;color:var(--tx);letter-spacing:-1px;margin-bottom:10px}
.login-logo span{color:var(--ac)}
.login-tagline{font-size:.9rem;color:var(--mu);margin-bottom:44px;line-height:1.7}
...
.back-btn{background:none;border:1px solid var(--b2);color:var(--tx2);border-radius:99px;padding:7px 18px;cursor:pointer;font-family:'DM Sans',sans-serif;font-size:.8rem;font-weight:600;transition:all .15s}
...
.stat-card-title{font-size:.65rem;color:var(--mu);text-transform:uppercase;letter-spacing:.14em;font-weight:700;margin-bottom:16px}
...
.rings-card{background:var(--s);border:1px solid var(--b);border-radius:var(--r2);padding:28px 24px 52px;margin-bottom:24px;display:flex;align-items:center;position:relative;overflow:hidden}
...
.f-stat-val{font-family:'DM Mono',monospace;font-weight:700;font-size:.82rem;line-height:1}
...
.time-win-msg{font-size:.75rem;color:var(--ac);font-weight:600;margin-top:5px}
...
.time-progress-bar-fill::after{content:'';position:absolute;right:-1px;top:50%;transform:translateY(-50%);width:14px;height:14px;background:#c8ff57;border-radius:50%;box-shadow:0 0 12px #c8ff57}
...
.task-timer-badge{font-family:'DM Mono',monospace;font-weight:700;font-size:.75rem;color:var(--ac);background:#c8ff5712;border:1px solid #c8ff5728;padding:3px 9px;border-radius:99px;flex-shrink:0;white-space:nowrap}
...
.task-done-badge{display:inline-flex;align-items:center;gap:7px;background:#34d39918;border:1px solid #34d39935;color:#34d399;border-radius:99px;padding:7px 18px;font-size:.8rem;font-weight:700;margin-bottom:24px}
...
.timer-status{font-size:.75rem;color:var(--mu);text-transform:uppercase;letter-spacing:.14em;font-weight:700;margin-bottom:28px}
.timer-card.running .timer-status{color:#c8ff5780}
.timer-btn{display:inline-flex;align-items:center;justify-content:center;gap:10px;border:none;border-radius:99px;padding:16px 40px;font-family:'Syne',sans-serif;font-weight:800;font-size:.95rem;cursor:pointer;transition:all .2s}
.timer-btn.start{background:var(--ac);color:#000;box-shadow:0 4px 28px #c8ff5745}
.timer-btn.start:hover{background:#d9ff70;transform:scale(1.06)}
...
.lock-in-btn{background:#c8ff5718;border:1px solid #c8ff57;color:#c8ff57;border-radius:10px;padding:10px 28px;cursor:pointer;font-family:'Syne',sans-serif;font-weight:700;font-size:.85rem;transition:all .2s;margin-top:4px}
.lock-in-btn:hover{background:#c8ff5730}
...
.t-stat-val{font-family:'DM Mono',monospace;font-weight:700;font-size:.95rem;color:var(--tx);margin-bottom:4px}
...
.action-btn{flex:1;border:none;border-radius:14px;padding:15px;font-family:'Syne',sans-serif;font-weight:700;font-size:.88rem;cursor:pointer;transition:all .18s}
...
.remind-opt:hover{border-color:var(--ac);color:var(--ac);background:#c8ff570a}
...
.add-btn:hover{background:#d9ff70;transform:scale(1.06)}
...
.sw.sel{border-color:#fff;transform:scale(1.25)}
...
.day-badge{font-family:'Syne',sans-serif;font-weight:700;font-size:.78rem;padding:4px 12px;border-radius:8px}
...
.momentum-title{font-size:.68rem;color:var(--mu);text-transform:uppercase;letter-spacing:.14em;font-weight:700}
.momentum-status{font-size:.75rem;font-weight:700}
...
/* Lock screen */
.lock-screen{position:fixed;inset:0;z-index:500;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px 24px;background:radial-gradient(ellipse at 50% 30%,#c8ff5708 0%,#050505 60%)}
...
.lock-task-name{font-family:'Syne',sans-serif;font-weight:800;font-size:clamp(1.3rem,4vw,2rem);color:var(--tx);margin-bottom:40px;max-width:520px;line-height:1.3;padding:0 20px}
...
.lock-unlock-btn{background:none;border:1px solid #ef444420;color:#ef444860;border-radius:12px;padding:12px 24px;cursor:pointer;font-family:'Syne',sans-serif;font-weight:700;font-size:.82rem;transition:all .2s}
.lock-unlock-btn:hover{background:#ef44440f;border-color:var(--red);color:var(--red)}
.lock-done-card{background:var(--s);border:1px solid #c8ff5730;border-radius:22px;padding:36px 32px;max-width:400px;width:100%}
.lock-done-title{font-family:'Syne',sans-serif;font-weight:800;font-size:1.8rem;color:var(--ac);margin-bottom:10px}
...
.lock-more-btn{background:var(--ac);color:#000;border:none;border-radius:14px;padding:15px;font-family:'Syne',sans-serif;font-weight:700;font-size:.92rem;cursor:pointer;transition:all .18s}
...
export default function App() {
  ...
  // ── TIMER: liveMs updates every 500ms when a timer is running ─────────────
  // This is the key fix — liveMs is App state, so updating it re-renders
  // everything including the timer display in the task detail view.
  const [liveMs, setLiveMs] = useState(Date.now());

  useEffect(() => {
    const running = tasks.find(t => t.timerRunning);
    if (!running) return;
    const iv = setInterval(() => setLiveMs(Date.now()), 500);
    return () => clearInterval(iv);
  }, [tasks]);

  // getLiveSecs uses liveMs from closure — updates whenever App re-renders
  const getLiveSecs = (task) => {
    const base = task.timerSeconds ?? 0;
    if (!task.timerRunning || !task.timerStartedAt) return base;
    return base + (liveMs - task.timerStartedAt) / 1000;
  };

  // Also used for lock countdown
  useEffect(() => {
    if (!isLocked || !lockEndTime || lockDone) return;
    if (liveMs >= lockEndTime) {
      setLockDone(true);
      playWin();
      setConfetti(true);
      if (user) setDoc(doc(db,"users",user.uid),{activeLock:null},{merge:true}).catch(()=>{});
    }
  }, [liveMs, isLocked, lockEndTime, lockDone]);

  // Keep activeTask synced with latest tasks
  useEffect(() => {
    if (activeTask) {
      const up = tasks.find(t => t.id === activeTask.id);
      if (up) setActiveTask(up);
    }
  }, [tasks]);
  ...
  // ── Load ─────────────────────────────────────────────────────────────────
  useEffect(() => {
    setLoaded(false); // Always reset first — prevents stale saves
    if (!user) {
      ...
      if (pinStep === 1) {
        ...
        if (next.length === 4) {
          if (next === pinInput) {
            ...
            setShowPinSetModal(false); setPinInput(""); setPinConfirm(""); setPinStep(1);
  ...
  const handlePinDel = () => {
    if (showPinSetModal) { if (pinStep===2) setPinConfirm(p=>p.slice(0,-1)); else setPinInput(p=>p.slice(0,-1)); }
    else if (showPinUnlock) setPinInput(p=>p.slice(0,-1));
  };

  const dismissLockDone = () => { setIsLocked(false); setLockDone(false); setLockEndTime(null); setLockedTaskId(null); setLockedTaskDk(null); };
  const lockMoreTime    = () => { setLockDone(false); setShowLockModal(true); };
  ...
  const completeTask = (remindDays=null) => {
    if (!activeTask) return;
    const dk = activeTaskDk, now = Date.now();
    playCheck();
    setTasks(prev => {
      let next = prev.map(t => {
        if (t.id !== activeTask.id) return t;
        ...
        return {...t, doneOn:[...(t.doneOn??[]),dateForDK(dk)], timerRunning:false, timerStartedAt:null, timerSeconds};
      });
      if (remindDays) {
        ...
        next = [...next, {id:Date.now(),text:activeTask.text,folderId:activeTask.folderId,recurring:false,day:futureDk,scheduledDate:dStr(future),done:false,timerSeconds:0,timerRunning:false,timerStartedAt:null,isReminder:true}];
      }
  ...
  const openRename = (e,folder) => { e.stopPropagation(); setRenamingFolder(folder); setRenameText(folder.name); setShowRenameModal(true); };
  const saveRename = () => {
  ...
  const goTask   = (task,dk,from) => {
    const now = Date.now();
    const ro = tasks.find(t=>t.timerRunning&&t.id!==task.id);
    if (ro) {
      const el = Math.floor((now-ro.timerStartedAt)/1000);
      setTasks(prev=>prev.map(t=>t.id===ro.id?{...t,timerRunning:false,timerStartedAt:null,timerSeconds:(t.timerSeconds??0)+el}:t));
    }
  ...
  // ── TaskRow (no hooks — safe to call inline) ──────────────────────────────
  const TaskRow = ({task,dk,color,from}) => {
    ...
    if (task.dueDate && !done) {
      ...
      dueBadge=<span style={{fontSize:".6rem",padding:"2px 7px",borderRadius:5,background:bg,color:col,fontWeight:700,flexShrink:0,whiteSpace:"nowrap"}}>{label}</span>;
    }
    return(
      <div className={`task-row${done?" done":""}${hasTime&&!done?" has-timer":""}${task.dueDate&&!done&&today>task.dueDate?" overdue":""}`}
        style={{"--rc":color}} onClick={()=>goTask(task,dk,from??view)}>
  ...
  // ── AddRow (has useState — must remain JSX component) ────────────────────
  const AddRow = ({dk,fid,placeholder}) => {
    ...
    const getDueOptions = () => {
      ...
      return [{value:null,label:"No deadline"},{value:taskStartDate,label:"Same day"},
        ...[1,3,7,14,30].map(days=>{const d=new Date(start);d.setDate(d.getDate()+days);
          return {value:dStr(d),label:days===1?"+1 day":days===7?"+1 week":days===14?"+2 weeks":days===30?"+1 month":`+${days} days`};})];
    };
    const submit = () => {
      const t=text.trim(); if(!t) return;
      const startDt=new Date(taskStartDate), dayKey=DAY_KEYS[(startDt.getDay()+6)%7];
      const base={id:Date.now(),text:t,folderId:fid??folders[0]?.id??null,timerSeconds:0,timerRunning:false,timerStartedAt:null,startDate:taskStartDate,dueDate:taskDueDate||null,day:dayKey};
      ...
      setText(""); setShowDates(false); inputRef.current?.focus();
    };
    return(
      <div className="add-area">
        ...
        {showDates&&!taskRecur&&(
          <div style={{background:"var(--s)",border:"1px solid var(--b2)",borderRadius:12,padding:14,marginTop:8,display:"flex",flexDirection:"column",gap:14}}>
            <div>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {startOptions.map(opt=>(
                  <button key={opt.value} onClick={()=>{setTaskStartDate(opt.value);setTaskDueDate(null);}}
                    style={{background:taskStartDate===opt.value?"var(--ac)":"var(--bg)",color:taskStartDate===opt.value?"#000":"var(--tx2)",border:`1px solid ${taskStartDate===opt.value?"var(--ac)":"var(--b2)"}`,borderRadius:8,padding:"5px 11px",cursor:"pointer",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".75rem"}}>
                    {opt.label}
                  </button>
                ))}
              </div>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {getDueOptions().map((opt,i)=>(
                  <button key={i} onClick={()=>setTaskDueDate(opt.value)}
                    style={{background:taskDueDate===opt.value?"#ef4444":opt.value===null&&taskDueDate===null?"var(--b2)":"var(--bg)",color:taskDueDate===opt.value?"#fff":"var(--tx2)",border:`1px solid ${taskDueDate===opt.value?"#ef4444":opt.value===null&&taskDueDate===null?"var(--mu)":"var(--b2)"}`,borderRadius:8,padding:"5px 11px",cursor:"pointer",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:".75rem"}}>
                    {opt.label}
                  </button>
                ))}
              </div>
            </div>
            <div style={{fontSize:".72rem",color:"var(--mu)",padding:"8px 10px",background:"var(--bg)",borderRadius:8,lineHeight:1.6}}>
              Starts <strong style={{color:"var(--tx)"}}>{startOptions.find(o=>o.value===taskStartDate)?.label??taskStartDate}</strong>
              {taskDueDate?<> · Due <strong style={{color:"#fb923c"}}>{getDueOptions().find(o=>o.value===taskDueDate)?.label??taskDueDate}</strong></>:<> · <span style={{color:"var(--mu)"}}>No deadline</span></>}
            </div>
          </div>
        )}
        {taskRecur&&(
  ...
  const DayMomentum = ({dk}) => {
    ...
    const taskPct=donePct(tasksForDay(dk),dk), diff=taskPct-dayPct;
    const timeStr=now.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
    let sl,sc,msg,bc;
    if(taskPct===100)  {sl="🎉 Complete";sc="#c8ff57";msg="All tasks done!";bc="#c8ff57";}
    else if(diff>=15)  {sl="🚀 Ahead";   sc="#c8ff57";msg=`${diff}% ahead of schedule`;bc="#c8ff57";}
    else if(diff>=-5)  {sl="⚡ On track";sc="#60a5fa";msg="Right on pace — keep it up";bc="#60a5fa";}
    else if(diff>=-20) {sl="⚠ Behind";  sc="#fbbf24";msg=`${Math.abs(diff)}% behind — push!`;bc="#fbbf24";}
    else               {sl="🔴 Lagging"; sc="#ef4444";msg="Focus up — time is moving fast";bc="#ef4444";}
    return(
      <div className="momentum-card">
        <div className="momentum-header">
          <div className="momentum-title">⚡ Day Momentum · {timeStr}</div>
          <div className="momentum-status" style={{color:sc}}>{sl}</div>
        </div>
        <div className="momentum-bars">
          <div className="momentum-bar-row">
            <span className="momentum-bar-lbl">Tasks done</span>
            <div className="momentum-bar-bg"><div className="momentum-bar-fill" style={{width:`${taskPct}%`,background:bc,boxShadow:`0 0 8px ${bc}60`}}/></div>
        ...
        <div className="momentum-msg" style={{color:sc+"cc"}}>{msg}</div>
      </div>
    );
  };
  ...
  const HomeView = () => {
    const dk=todayKey();
    const tMonth=tasksCompletedThisMonth(),hMonth=secsWorkedThisMonth()/3600,hWeek=secsWorkedThisWeek()/3600;
    ...
    const enriched=[...folders].map(f=>{
      const todayFT=tasksForDay(dk).filter(t=>t.folderId===f.id);
      const doneToday=todayFT.filter(t=>isDone(t,dk)).length,todayCount=todayFT.length;
      let wd=0,wdn=0; DAY_KEYS.forEach(d=>{const dayF=tasksForDay(d).filter(t=>t.folderId===f.id);wd+=dayF.length;wdn+=dayF.filter(t=>isDone(t,d)).length;});
      const weekPctF=wd>0?Math.round(wdn/wd*100):0;
      const totalSecs=folderTasks(f.id).reduce((s,t)=>s+(t.timerSeconds??0),0);
      return {f,todayCount,doneToday,weekDue:wd,weekDone:wdn,weekPctF,totalSecs,hasTasksToday:todayCount>0};
    });
      ...
      <div className="home-layout">
        <div>
          ...
          {folders.length===0
            ...
            :<div className="folders-list">
              {[...active,...inactive].map((e,idx)=>{
                const {f,todayCount,doneToday,weekDue,weekDone,weekPctF,totalSecs,hasTasksToday}=e;
                const dimmed=!hasTasksToday&&active.length>0;
                return(
                  <React.Fragment key={f.id}>
                    {idx===active.length&&inactive.length>0&&active.length>0&&(
                    ...
                    <div className="folder-row" style={{"--fc":dimmed?"#444":f.color,opacity:dimmed?0.45:1}} onClick={()=>goFolder(f.id)}>
                      ...
                      <div className="folder-row-main">
                        ...
                        <div className="folder-row-bar-bg"><div className="folder-row-bar-f" style={{width:`${weekPctF}%`,background:dimmed?"#444":f.color}}/></div>
                      </div>
                      ...
                      <button onClick={ev=>openRename(ev,f)} style={{background:"none",border:"none",color:"var(--mu)",cursor:"pointer",fontSize:".85rem",padding:"4px 6px",borderRadius:6,flexShrink:0}}>✏️</button>
        ...
        <div className="stats-col">
          ...
          <div className="stat-card">
            ...
            <div className="stat-big" style={{color:"#fb923c",fontSize:"2.4rem"}}>{fmtTimer(secsTracked(dk))}</div>
  ...
  const DayView = () => {
    ...
    const dt=tasksForDay(dk),done=dt.filter(t=>isDone(t,dk)).length,pct=donePct(dt,dk),st=secsTracked(dk);
    const budgetSecs=hoursFor(dk)*3600,pctWorked=Math.min(100,(st/budgetSecs)*100),workedMins=Math.floor(st/60),budgetHrs=hoursFor(dk);
    ...
    const milestones=[{pct:25,label:fmtHrs(budgetHrs*.25)},{pct:50,label:fmtHrs(budgetHrs*.5)},{pct:75,label:fmtHrs(budgetHrs*.75)},{pct:100,label:`${budgetHrs} hrs`}];
    let winMsg=null;
      ...
      <div className="page">
        ...
        <div className="time-progress-card">
          <div className="time-progress-top">
            ...
            <div className="time-progress-goal">Goal: {budgetHrs} hrs
              <button style={{background:"none",border:"none",color:"var(--mu)",cursor:"pointer",fontSize:".7rem",marginLeft:8,fontWeight:600,textDecoration:"underline"}} onClick={()=>openHours(dk)}>change</button>
            </div>
          </div>
  ...
  const TaskDetailView = () => {
    ...
    const dk   = activeTaskDk;
    const done = isDone(task,dk);
    const secs = getLiveSecs(task); // uses liveMs — updates every 500ms ✅
        ...
        <div className={`timer-card${isRunning?" running":""}`}>
          ...
          {!done&&(
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
              ...
              <button className="lock-in-btn" onClick={openLockFlow}>🔒 Lock In</button>
            </div>
          )}
        ...
        {!done&&showRemind&&(
          <div className="remind-section">
            ...
            <div style={{textAlign:"center",marginTop:12}}>
              <button style={{background:"none",border:"none",color:"var(--mu)",cursor:"pointer",fontSize:".8rem"}} onClick={()=>setShowRemind(false)}>← Back</button>
            </div>
          </div>
        )}
      </div>
  ...
  // ── All Tasks View (has useState — stays as JSX component) ────────────────
  const AllTasksView = () => {
        ...
        {sortBy==="date"?groups.map(g=>(
          <div className="day-section" key={g.key}>
            <div className="day-section-hdr">
              ...
              <span style={{fontSize:".72rem",color:"var(--mu)"}}>{g.date}</span>
              <span style={{marginLeft:"auto",fontSize:".72rem",color:"var(--mu)",fontWeight:600}}>{g.items.filter(i=>i.done).length}/{g.items.length}</span>
            </div>
            {g.items.map((item,idx)=><TaskRow key={`${item.task.id}-${idx}`} task={item.task} dk={item.dk} color={item.folder?.color??"var(--ac)"} from="all"/>)}
          </div>
        )):groups.map(g=>(
          <div className="day-section" key={g.key}>
            <div className="day-section-hdr">
            ...
            {g.items.map((item,idx)=><TaskRow key={`${item.task.id}-${idx}`} task={item.task} dk={item.dk} color={g.color} from="all"/>)}
          </div>
        ))}
      </div>
    );
  ...
  if(authLoading) return(
    ...
    <div style={{minHeight:"100vh",background:"#080808",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{color:"#333",fontSize:".9rem",fontWeight:600}}>Loading…</div>
    </div></>
  );
  ...
  // ── Main render ───────────────────────────────────────────────────────────
  // Views called as plain functions (not JSX) — prevents remounting on every
  // liveMs update which would make buttons unresponsive.
  const lockSecsLeft = lockEndTime ? Math.max(0,(lockEndTime-liveMs)/1000) : 0;
  const lockedTask   = tasks.find(t=>t.id===lockedTaskId);
  const lockPctLeft  = lockDuration>0 ? (lockSecsLeft/(lockDuration*60))*100 : 0;
  const lockIsUrgent = lockSecsLeft < 60;
    ...
    {isLocked&&(
      lockDone?(
        ...
        <div className="lock-screen">
          ...
          <div className={`lock-countdown${lockIsUrgent?" urgent":""}`}>{fmtTimer(lockSecsLeft)}</div>
          <div className="lock-countdown-lbl">remaining</div>
          <div className="lock-prog-wrap">
            <div className="lock-prog-bg">
              <div className={`lock-prog-fill${lockIsUrgent?" urgent":""}`} style={{width:`${lockPctLeft}%`}}/>
            </div>
          </div>
          <div className="lock-working">Working for <strong>{fmtTimer(getLiveSecs(lockedTask??{}))}</strong></div>
    ...
    {showLockModal&&!isLocked&&(
      <div className="overlay" onClick={()=>setShowLockModal(false)}>
        <div className="modal" onClick={e=>e.stopPropagation()}>
          ...
          <div style={{fontSize:".82rem",color:"var(--mu)",marginBottom:18,lineHeight:1.6}}>
    ...
    {showPinSetModal&&(
      <div className="overlay">
        <div className="modal" style={{maxWidth:320}}>
          ...
          <div style={{fontSize:".8rem",color:"var(--mu)",textAlign:"center",marginBottom:20}}>
    ...
    {showHoursModal&&(
      <div className="overlay" onClick={()=>setShowHoursModal(false)}>
        <div className="modal" onClick={e=>e.stopPropagation()}>
          ...
          <div style={{fontSize:".8rem",color:"var(--mu)",marginBottom:18}}>Your daily time budget. Tracks against actual time worked.</div>
          <div className="modal-btns">
